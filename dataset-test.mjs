...
const FEAR = ['αποκλεισμος','απενεργοποι','αναστολη','παγωμενος','μπλοκαρε','blocked','suspended','disabled','κλειδωμα','κλειδωθηκε','διαγραφη','απαγορευση','παραβιαση','unauthorized','illegal','παρανομη','ιος','μολυνθηκε','μολυνση','χακαρ','hacked','επιθεση','κινδυνος','διαρροη','κλεψιμο','κλεφτης','κλεφτες','διακοπη','φραγη','περιορισμος','θα περιοριστει','θα ληξει','υποπτη δραστηριοτητα','υποπτη συναλλαγη','νεα συσκευη','αντικατασταση sim']
...
function analyzeText(text) {
  ...
  // 12b. Gov/bank keyword in unknown domain
  const govD = unknownDomains.filter(d => d.split(/[-_.]/).includes('gov') || d.startsWith('gov'))
  if (govD.length > 0) { add(signals,'Domain μιμείται κυβερνητική υπηρεσία',30,govD.join(',')); totalScore += 30 }
  const bankD = unknownDomains.filter(d => d.includes('bank'))
  if (bankD.length > 0) { add(signals,'Domain μιμείται τράπεζα',25,bankD.join(',')); totalScore += 25 }
